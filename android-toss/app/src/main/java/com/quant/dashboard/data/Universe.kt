...
/**
 * 종목 이름 검색용 유니버스 캐시 — `GET /api/v1/stocks/all` (미국 마켓만).
 *
 * 티커를 정확히 몰라도 종목을 추가할 수 있게 한다. 토스는 **미국 종목도 한글 이름**을 주므로
 * ("애플" → AAPL) 한글·영문·티커 어느 쪽으로 쳐도 찾힌다.
 *
...
object Universe {
    ...
    /** 사용자 결정(2026-08-29): 미장만 다룬다 — 국내 마켓은 받지 않는다. */
    private val MARKETS = listOf("NASDAQ", "NYSE", "AMEX")
    ...
    private fun save() {
        ...
        try {
            ...
            f.writeText(JSONObject().put("date", date).put("items", arr).toString())
        } catch (e: Exception) {}
    }
