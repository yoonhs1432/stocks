...
/**
 * 토스증권 Open API 자격증명 — 기기 내부 **암호화** 저장(EncryptedSharedPreferences).
 *
...
object BrokerCreds {
    ...
    /** 계좌 식별 키. 미연결이면 -1. */
    fun accountSeq(): Long = prefs?.getLong(K_ACCOUNT_SEQ, -1L) ?: -1L
    fun accountNo(): String = prefs?.getString(K_ACCOUNT_NO, "").orEmpty()

    fun hasKeys(): Boolean = appKey().isNotBlank() && appSecret().isNotBlank()

    /** 키 + 계좌까지 모두 준비되어 계좌 조회가 가능한 상태. */
    fun isLinked(): Boolean = hasKeys() && accountSeq() >= 0

    /** 키 저장. 키가 바뀌면 계좌·토큰 캐시를 무효화한다. */
    fun saveKeys(appKey: String, appSecret: String) {
        val changed = appKey.trim() != this.appKey() || appSecret.trim() != this.appSecret()
        prefs?.edit()
            ?.putString(K_KEY, appKey.trim())
            ?.putString(K_SECRET, appSecret.trim())
            ?.apply()
        if (changed) {
            prefs?.edit()?.remove(K_ACCOUNT_SEQ)?.remove(K_ACCOUNT_NO)?.apply()
            TossApi.clearToken()
        }
    }

    /** 연결 테스트로 조회한 계좌 확정. */
    fun saveAccount(seq: Long, no: String) {
        prefs?.edit()?.putLong(K_ACCOUNT_SEQ, seq)?.putString(K_ACCOUNT_NO, no)?.apply()
    }
    ...
    /** 화면 표시용 마스킹 (앞 4자만). 원문은 어디에도 출력하지 말 것. */
